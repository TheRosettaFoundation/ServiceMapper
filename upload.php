...
function view($out){
	...
	foreach ($nodes as $node) {
		...
		if ($node->hasChildNodes()){
			...
			print '<tr class="row" id="src"><td>source</td><td colspan="3" rowspan="1">'.htmlentities($source, ENT_QUOTES, 'UTF-8')
					 .'</td></tr><tr id="tgt"><td>target</td><td colspan="3" rowspan="1" class="dblclick" id="'.$name.'">'.$target.'</td></tr>';
	...
	foreach ($nodes as $node){
		...
		for ($i = 0; $i < $length; ++$i){$name =$node->attributes->item($i)->name;
